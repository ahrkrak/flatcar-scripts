...
# Flatcar: this package shadows the identical ebuild in portage-stable purely
# to carry files/sandbox-2.46-x86_64-degrade-when-regs-unreadable.patch, which
# keeps libsandbox from aborting the build when the x86_64 register file is not
# readable via ptrace() -- as is the case when the amd64 SDK container runs
# under Apple Rosetta on an arm64 host.  Without it, every static ELF (i.e.
# every Go binary) dies with "unknown x86_64 (CS) personality".
#
# When bumping sandbox in portage-stable, re-copy the ebuild here, refresh the
# patch, and drop this package entirely if the fix lands upstream.
...
PATCHES=(
	"${FILESDIR}"/${PN}-2.46-x86_64-degrade-when-regs-unreadable.patch
)
